Gsphere <-
function(X, win=sphwin(type="sphere"), rvals=seq(from=0, to=pi, length=512), ...) {
...
rad <- win$rad
if(length(X)==0) {stop("Cannot estimate G function of an empty set")} else {
D <- nndistsph(X=X)
...
f <- bind.fv(f, data.frame(rsmodif=frsmod), labl="%s[bordmodif](r)", desc="modified border corrected estimate of %s")
